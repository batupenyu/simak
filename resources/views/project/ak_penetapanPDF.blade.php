{{-- @if ($akintegrasi != null) --}}
    {{-- @forelse ($periodeAK as $data)  --}}
        @php
            ...
            $tglPenetapan = \Carbon\Carbon::parse($akintegrasi->tgl_penetapan);
        ...
        <p style="text-align: center">
            <b>PENETAPAN ANGKA KREDIT<br>
                NOMOR : 800/ ....... /SMKN 1 Kb/Dindik/{{ Carbon\Carbon::parse($akintegrasi->tgl_penetapan)->translatedFormat('Y')}}/PAK</b>
        </p>
        ...
        <tr>
            ...
            <td style="text-align: right">
                ...
                @php
                    ...
                    if ($startDateCarbon->isWeekend() || isHoliday($startDateCarbon, $holidays)) {
                        $startDateCarbon = $startDateCarbon->addDay();
                    }
                @endphp
                {{ $startDateCarbon->translatedFormat('d F') }}
                @php
                    $endDateCarbon = Carbon\Carbon::parse($end_date);
                    if ($endDateCarbon->isWeekend() || isHoliday($endDateCarbon, $holidays)) {
                        $endDateCarbon = $endDateCarbon->addDay();
                    }
                @endphp
                - {{ $endDateCarbon->translatedFormat('d F Y') }}
            </td>
        </tr>
        @else @endif
    </table>
...
@forelse ($periodeAK as $kredit)
        ...
        <?php
            ...
            if (isset($kredit->user) && $kredit->user->pangkat_gol == 'Penata Muda, III/a') {
                $jenjang = 'Ahli Pertama';
            } elseif (isset($kredit->user) && $kredit->user->pangkat_gol == 'Penata Muda TK.I, III/b') {
                $jenjang = 'Ahli Pertama';
            } elseif (isset($kredit->user) && $kredit->user->pangkat_gol == 'Penata, III/c') {
                $jenjang = 'Ahli Muda';
            } elseif (isset($kredit->user) && $kredit->user->pangkat_gol == 'Penata TK.I, III/d') {
                $jenjang = 'Ahli Muda';
            } elseif (isset($kredit->user) && $kredit->user->pangkat_gol == 'Pembina, IV/a') {
                $jenjang = 'Ahli Madya';
            } else {
                echo '';
            }
        ...
        <tr>
            ...
            <?php
            $date1 = $akintegrasi->tgl_awal_penilaian;
            $date2 = $akintegrasi->tgl_akhir_penilaian;
            $predikat = $akintegrasi->predikat;

            // Define jenjang and koe based on pangkat_gol
            if ($akintegrasi->user->pangkat_gol == 'Penata Muda, III/a' || $akintegrasi->user->pangkat_gol == 'Penata Muda TK.I, III/b') {
                $jenjang = 'Ahli Pertama';
                $koe = 12.5;
            } elseif ($akintegrasi->user->pangkat_gol == 'Penata, III/c' || $akintegrasi->user->pangkat_gol == 'Penata TK.I, III/d') {
                $jenjang = 'Ahli Muda';
                $koe = 25;
    ...
    <table border="0" cellpadding="2">
        <tr>
            <td style="width: 60%">ASLI disampaikan dengan hormat kepada: <br>Jabatan Fungsional yang bersangkutan. <br><br>
...
@php
    $tglttd = \Carbon\Carbon::parse($endDateCarbon)->addDays(1);
    $tglttd->locale('id_ID');
...
@endphp
            <td style="width: 40%">Ditetapkan di Koba. <br>Pada tanggal, {{ $tglttd->translatedFormat('d F Y') }}. <br>Pejabat Penilai Kinerja,
                <br>
                <br>
                <br>
                <br>{{$akintegrasi->penilai->nama}}
